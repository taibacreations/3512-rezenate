...
const Philosophy = ({ data }: PhilosophyProps) => {
  ...
  useEffect(() => {
    const ctx = gsap.context(() => {
      gsap.set(
        [headingRef.current, para1Ref.current, para2Ref.current, para3Ref.current, quoteBoxRef.current],
        { autoAlpha: 0 },
      );
      ...
      const createTrigger = () => {
        ...
        tl.fromTo(headingRef.current, { autoAlpha: 0, y: 50 }, { autoAlpha: 1, y: 0, duration: 1.2, ease: "expo.out" })
      ...
      <div className="max-w-[1480px] mx-auto xl:px-10 md:px-6 px-4 xl:pt-[10vh] md:pt-[10vh] pt-[50px] relative z-30">
        <div className="xl:max-w-[714px] lg:max-w-[620px] max-w-[550px] relative z-20">
          ...
          <div className="max-w-[434px]">
            <p ref={para1Ref} style={{ opacity: 0 }} className="font-outfit font-normal 2xl:text-[30px] xl:text-[26px] text-[22px] leading-[114%] text-[#0B0730] mt-[3vh]">{para1}</p>
            <p ref={para2Ref} style={{ opacity: 0 }} className="font-outfit font-normal 2xl:text-[30px] xl:text-[26px] text-[22px] leading-[114%] text-[#0B0730] mt-[3.5vh]">{para2}</p>
            <p ref={para3Ref} style={{ opacity: 0 }} className="font-outfit font-normal 2xl:text-[30px] xl:text-[26px] text-[22px] leading-[114%] text-[#0B0730] mt-[3.5vh]">{para3}</p>
          </div>

          <div ref={quoteBoxRef} style={{ opacity: 0 }} className="mt-[4vh]">
            <div className="max-w-[368px] border-l-2 border-black pl-[1vw]">
              <h5 className="font-outfit 2xl:text-[22px] xl:text-[24px] lg:text-[22px] text-[18px] leading-[114%] font-normal text-[#0B0730]">{quoteText}</h5>
              <h4 className="font-outfit font-semibold 2xl:text-[20px] lg:text-[18px] text-[16px] leading-[114%] text-[#0B0730] mt-[1vh]">— {quoteAuthor}</h4>
            </div>
          </div>
        </div>
      </div>
