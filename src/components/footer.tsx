...
const Footer = () => {
  ...
  // One-time logo entrance — paths stagger in and stay, no infinite loop
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    const ctx = gsap.context(() => {
      ...
      gsap.set(headingRef.current, {
        ...
        // letterSpacing animation removed — too stylized
      });
      gsap.set(paraRef.current, { opacity: 0, y: 16 }); // reduced from 24
      gsap.set(copyrightRef.current, { opacity: 0, y: 10 }); // reduced from 16
      ...
      tl
        // Background veil lifts slowly — the reveal IS the animation
        .to(bgOverlayRef.current, {
          opacity: 0,
          duration: 2.2,   // slowed from 1.6
          ease: "power2.out",
        })
        ...
        .to(
          paraRef.current,
          { opacity: 1, y: 0, duration: 1.0, ease: "power2.out" }, // slowed from 0.75
          "-=0.7"
        )
        ...
        .to(
          ...
          { opacity: 1, y: 0, duration: 0.9, ease: "power2.out" }, // slowed from 0.6
          "-=0.5"
        );
    }, footerRef);
  ...
  return (
    ...
    >
      {/* Dark veil that lifts on entrance */}
      <div
        ref={bgOverlayRef}
      ...
      >
        ...
        ></path>
        <path
          d="M40.7108 143.906V103.203H0L40.7108 143.906Z"
          fill="white"
        ></path>
      </svg>
      ...
      <div className="text-center relative z-20 lg:px-0 px-4 md:px-6">
        ...
        >
          10% of every retainer supports a cause our clients care about. We also
          make a matching donation to a charity chosen by their new leader,
          because good business should always leave the world better than it
          found it.
        </p>
      </div>
