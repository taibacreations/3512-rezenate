...
const FALLBACK_VALUES = [
  ...
  },
  {
    name: "Rezolutionary",
    shortDescription:
      "We believe everyone has the power to shape what happens next.",
    longDescription:
...
const Section3 = ({ data }: { data: ValuesData }) => {
  const heading =
    data?.heading ?? "The way we work should reflect the way we live.";
  const values: ValueItem[] = data?.items?.length
    ? data.items
    : FALLBACK_VALUES;
  ...
  useEffect(() => {
    ...
    const ctx = gsap.context(() => {
      ...
      gsap.set(overlayLineRef.current, {
        scaleX: 0,
  ...
  const CardBackground = ({ index }: { index: number }) => (
    <>
      ...
      <div
        ...
        style={{
          background:
            "radial-gradient(circle at 60% 30%, rgba(149,100,244,0.35) 0%, rgba(149,100,244,0.0) 65%)",
        }}
      />
      <div
  ...
  return (
    <section
      id="section3"
      ref={sectionRef}
      className="2xl:px-16 xl:px-10 md:px-6 px-4 relative mt-[-47vh]"
    >
      <div>
        <div
          ref={headingRef}
          className="text-center 2xl:max-w-[819px] xl:max-w-[780px] max-w-[680px] mx-auto"
        >
          <div
            ref={lineRef}
      ...
      {activeValue && (
        ...
        >
          ...
          <div
            ...
            className="relative z-10 max-w-[620px] text-center text-white px-4"
          >
